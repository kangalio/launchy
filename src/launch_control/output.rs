...
impl Output {
    ...
    pub fn light_multiple(
        &mut self,
        template: impl Into<Template>,
        pads: impl IntoIterator<
            Item = impl std::borrow::Borrow<(Button, Color, DoubleBufferingBehavior)>,
        >,
    ) -> Result<(), crate::MidiError> {
    ...
    // pub fn toggle(&mut self, template: impl Into<Template>, button: Button, on: bool) ->
    // anyhow::Result<()> {     let value = if on { 127 } else { 0 };
    //     self.send(&[240, 0, 32, 41, 2, 10, 123, template.into().0, button.as_index(), value,
    // 247]) }
    ...
    pub fn turn_on_all_leds(
        ...
        self.send(&[0xB0 + template.into().0, 0, brightness_code])
    }
    ...
    pub fn control_double_buffering(
        ...
        self.send(&[0xB0 + template.into().0, 0, last_byte])
    }
    ...
    pub fn reset(&mut self, template: impl Into<Template>) -> Result<(), crate::MidiError> {
        self.turn_on_all_leds(template, Brightness::Off)
    }
